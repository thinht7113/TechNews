    ...
    setup() {
        ...
        const handleUpload = async (e) => {
            ...
            try {
                ...
                if (res.ok) {
                    Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Upload thÃ nh cÃ´ng', showConfirmButton: false, timer: 1500 });
                    fetchImages();
                }
            } catch (err) { Swal.fire('Lá»—i', 'Upload tháº¥t báº¡i', 'error'); }
            finally { uploading.value = false; e.target.value = ''; }
        };

        const copyUrl = (url) => {
            navigator.clipboard.writeText(url);
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'ÄÃ£ copy URL', showConfirmButton: false, timer: 1000 });
        };
    ...
    template: `
        <div>
            <div class="flex justify-between items-center mb-6">
                 <h2 class="text-2xl font-bold text-black">ThÆ° viá»‡n Media</h2>
            ...
            <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                <div v-for="img in images" :key="img.name" class="p-2 border rounded bg-white group relative hover:shadow-lg transition">
                    ...
                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center gap-2 rounded">
                        ...
                        <button @click="deleteImage(img.url)" class="bg-red-500 text-white text-xs font-bold px-3 py-1 rounded hover:bg-red-600">XÃ³a</button>
                    </div>
                </div>
            </div>
            
            <div v-if="images.length === 0 && !loading" class="text-center py-12 text-gray-500 border-2 border-dashed rounded-lg">
                ChÆ°a cÃ³ áº£nh nÃ o. HÃ£y upload ngay!
            </div>
        </div>
    `
};
